...
export class GerenteComponent {
  ...
  constructor(private reportService : ReportService,
    ...
    this.reportService.reportGerente2(this.bookstoreCode)
      .subscribe((result)=>{
        if(result.status){
    ...
    this.reportService.reportGerente1(this.bookstoreCode)
      .subscribe((result)=>{
        if(result.status){
          for (let i = 0; i < result.data.length; ++i) {
            let temp={
    ...
    this.reportService.reportGerente3(this.bookstoreCode)
      .subscribe((result)=>{
        if(result.status){
          for (let i = 0; i < result.data.length; ++i) {
            let temp={
    ...
    this.reportService.reportGerente4(this.bookstoreCode)
      .subscribe((result)=>{
        if(result.status){
          for (let i = 0; i < result.data.length; ++i) {
            let temp={
